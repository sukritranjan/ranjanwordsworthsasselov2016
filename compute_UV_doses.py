...
wav_leftedges, wav_rightedges, rugheimer_surface_int=get_UV('./TwoStreamOutput/rugheimer_earth_epoch0_reproduce_rugheimer.dat')

#Compute base doses
intrad100_165_base=integrated_radiance(wav_leftedges, wav_rightedges, rugheimer_surface_int, 100, 165.) #This measures the flux vulnerable to activity
...
if calculate_all_doserates:
	...
	for filename in file_list:
		...
		line=np.array([intrad100_165/intrad100_165_base,intrad200_300/intrad200_300_base, umpgly_193/umpgly_193_base, umpgly_230/umpgly_230_base, umpgly_254/umpgly_254_base, tricyano254/tricyano254_base, tricyano300/tricyano300_base])

		#Save output
		outputfile='./DoseRates/dose_rates_'+filename
		f=open(outputfile,'w')
		f.write('All Dosimeters Normalized to Space Radiation Case\n')
		np.savetxt(f, line[np.newaxis], delimiter='	', fmt='%s', newline='\n', header='Radiance (100-165 nm) & Radiance (200-300 nm) & UMP Gly Cleavage (lambda0=193nm) & UMP Gly Cleavage (lambda0=230nm) & UMP Gly Cleavage (lambda0=254nm) &  CuCN3 Photoionization (lambda0=254 nm) & CuCN3 Photoionization (lambda0=300 nm)\n')
		f.close()
